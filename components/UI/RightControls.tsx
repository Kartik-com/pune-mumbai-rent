export default function RightControls({
  ...
  return (
    <div className="fixed right-4 top-[84px] flex flex-col gap-[10px] z-[1000] pointer-events-auto">
      {/* Style Toggle */}
      <button onClick={onToggleStyle} title="Switch Map Style" className="w-[42px] h-[42px] rounded-[10px] glass glass-hover flex items-center justify-center text-text2 hover:text-accent shadow-2xl transition-all">
        {mapStyle === 'dark' ? (
      ...
      {/* Heatmap */}
      <button onClick={onToggleHeatmap} title="Rent Heatmap" className={`w-[42px] h-[42px] rounded-[10px] flex items-center justify-center shadow-2xl transition-colors ${showHeatmap ? 'bg-orange-500 text-white border-none' : 'glass glass-hover text-text2 hover:text-text1'}`}>
